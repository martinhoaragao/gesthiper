...
\begin{lstlisting}
struct node{
	int qnt; //Valor que no primeiro nodo diz quantos produtos daquela inicial existem, e que no último nodo marca o produto como comprado ou não
	char code[6]; //Código a guardar
...
\begin{lstlisting}
struct listnode{
	int qnt; //Valor que diz quantos produtos estão no array de Strings
	char **codes; //Array de Strings que guarda códigos de Produtos
};
\end{lstlisting}
